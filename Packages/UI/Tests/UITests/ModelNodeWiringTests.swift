...
struct ModelNodeWiringTests {
    ...
    @Test func actualWorkbenchResetsNodeSelectionWhenClosingAndOpeningAnotherProject() async throws {
        try await withFixture { service, model, page, tags, _, engine, project in
            ...
            let other = project.deletingLastPathComponent().appendingPathComponent("Other.dproject")
            await service.createProject(at: other)
            try #require(service.manifest != nil && service.projectURL == other)
            settle(host)
            #expect(page.pane == .creations && page.selectedNodeID == nil)
