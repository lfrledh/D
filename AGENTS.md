...
D-F03／D-M01 已验收：现有 UI package 内含真实 DWorkbench 服务 target 和模型安装管理，保留单项目流程。下一项 D-W02 多文档及格式迁移按当前行动指南细化。快速草稿、远程 API、对外服务、RAW 等按目标表启动，不因规划存在就实现空入口或框架。信息架构见 ADR 0007，服务演进见 ADR 0008。
...
- 当前项目 v1 保持兼容。界面、项目格式、内部 Swift 接口和未来外部协议分别演进；迁移须有数据保护和测试。
