...
D-F03／D-M01 已验收：现有 UI package 内含真实 DWorkbench 服务 target 和模型安装管理，保留单项目流程。已确认总体战略见 docs/PRODUCT_STRATEGY.zh-CN.md；当前 D-W02 / D-W04a 可持续探索与比较见 docs/EXPLORATION_STAGE_PLAN.zh-CN.md。快速草稿、远程 API、对外服务、RAW 等按目标表启动，不因规划存在就实现空入口或框架。信息架构见 ADR 0007，服务演进见 ADR 0008。
