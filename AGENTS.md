...
模型真实要求、适配支持、实测范围、硬件推荐和用户选择/准入分别表达；M4/16GiB只代表开发验收机，small/512/2048不作为产品永久上限。声明必须对应实际执行，不能静默忽略条件或改变精度。D-ALIGN-01已验收并推送；D-VIDEO-V0-01薄视频后端已验收推送；2026-09-14用户批准[D-VIDEO-WORKBENCH-01](docs/tasks/D-VIDEO-WORKBENCH-01.md)基础视频工作台闭环。先完成各模态基础能力，再实现能力组合。新权重/依赖须获具体批准，模型与媒体验收分开；本批UI只接基础闭环，不扩成时间线/编辑器或跨模态组合。
