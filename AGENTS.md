...
模型真实要求、适配支持、实测范围、硬件推荐和用户选择/准入分别表达；M4/16GiB只代表开发验收机，small/512/2048不作为产品永久上限。声明必须对应实际执行，不能静默忽略条件或改变精度。D-ALIGN-01已验收并推送；2026-09-14用户批准[D-VIDEO-V0-01](docs/tasks/D-VIDEO-V0-01.md)薄视频后端实施。新权重/依赖须获具体批准，模型与媒体验收分开；不沿历史提案扩成视频UI/编辑器。
