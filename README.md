...
视频V0薄后端已完成本地阶段验收：Wan2.1 T2V-1.3B通过统一运行时生成无声H.264 MP4，保留实际参数、模型/精度及完整媒体检查记录；普通工作台视频入口尚未开放。M4/16GiB真实832×480、17帧/50步样本约27.8分钟，MLX峰值18.294GiB；内存指导值不是硬上限，长视频/其他Mac仍待测。调用、单点资源校准与限制见[视频后端指南](Backends/Video/README.md)，完整受测版本和提交/推送见[本阶段记录](docs/tasks/D-VIDEO-V0-01.md)。
