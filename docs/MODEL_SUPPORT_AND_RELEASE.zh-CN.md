...
核实日期：2026-09-23；生产快照`b334920907de0324bf3e0146bb78433742356a6c`。以下模型验收复用明确历史版本；2026-09-24增量核对165c544中的模型说明与服务入口，最新收尾不代表重跑全模态。源/产物/候选身份见[当前行动](CURRENT_ACTIONS.zh-CN.md)，产品承诺见[目标](PRODUCT_GOALS.zh-CN.md)。静态代码中的旧awaiting标签若与运行记录不同，登记为展示/能力目录债，不在文档整理时改生产逻辑。
...
发布后保留更多模型/LoRA、复杂蒙版/精修、长视频/完整DAW/高级谱面、高级通用编程/任意图兼容、RAW及更多容器、手机捕捉、远程API/C2PA等；不要求首发全做，不因一个模型兼容问题无限追加。旧先全模态再组合排期已替代；当前授权与停止位置只看[CURRENT_ACTIONS](CURRENT_ACTIONS.zh-CN.md)。具体节点粒度和首发交互等待用户方案，目标表不自动批准实施。
...
- 源R9实际验证`8fb7d48e14ba4e140177925905ddb8130751617d`，到b334只有文档。参考full50使用CORE冻结外部脚本/输入清单，guard及data/result位于`D-Development/AgentTrials/D-CORE-CLOSE-01/run-20260921T175000Z/video/full50/`；50步、原attention容差及首步对照、取消恢复通过；约39247.6秒，释放后PyTorch MPS active为0，driver记录不是整个系统零占用。输出仅FP32 latent，摘要与当时读取依据在整理任务。
