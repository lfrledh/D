...
核实日期：2026-09-22。状态盘点基线 `368751084ab94a58b4d97dc96ad5c7c0948d2419`；本次复用既有证据，未重新执行全模态验收。最新I2V R9结果见[任务](tasks/D-VIDEO-I2V-01.md#r9-results)及CURRENT_ACTIONS。本文集中消除旧指南中“待实测”的过时概括，不倒改历史。公开发行范围仍以[产品目标](PRODUCT_GOALS.zh-CN.md#发布前发布后归属2026-09-15r2026-09-15.1)为准。
...
| 首图视频：Wan-AI/Wan2.2-TI2V-5B | 隔离候选；R9已实现GPU注意力与全30块流式部件，固定1216×736/121帧完整正负首步通过 | 825权重及重复/取消恢复通过；完整一步约14分钟。没有50步全片/画质/D适配通过，不能写源App支持I2V；本次结果不代表所有Mac配置 |
...
| 2 新UI＋一条真实组合 | 项目/资产/候选/保存等服务可复用；PNG配方基础已有 | 用户专项UI设计；实际组合例如文字整理生成条件→参考图候选→人工采用/保存来源。统一App内多个按钮分别成功不等于组合已验；输入法/缩放/键盘/可访问性/取消/保存需最终界面验收 |
| 3 公开能力的模型准备与许可 | 下载校验/恢复/重连及部分离线引擎部署已有 | 所有首发模型自助获取或明确外部准备；固定发行清单逐项核对源码、权重、分词器、声库、vocoder、实际随包、下载分发和演示宣传所用素材的使用/分发条件及归属；用户输入资格声明另行表达。SwiftF0权重unknown须解决或明确排除，若因此缩减既定HUM首发范围需明确产品决定，改为用户自备也不自动补足权利依据；资格框不创造许可。SA3/Gemma、MRT2、Qixuan/BigVGAN不能互相代替条款 |
...
发布后保留更多模型/LoRA、复杂蒙版/精修、长视频/完整DAW/高级谱面、自由工作流/布局、RAW及更多容器、手机捕捉、远程API/C2PA等；不要求首发全做，不因一个模型兼容问题无限追加。下一阶段仍从当前有限后端门槛往统一基础出口推进，具体以阶段回执及用户批准为准。
