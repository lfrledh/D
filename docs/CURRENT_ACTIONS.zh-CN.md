...
准备基线 `9fde96c625e86add2de8acb5e47ac9a52b71a5dc`；两项受限 Worker 独立实现数据与视图，Lead 接线/审阅/串行验证。当前在隔离分支验收；最终受测版本与接纳结果以任务末尾和外部 final-receipt 为准。参数为说明，不会提交执行；标签只写本机偏好，独立测试使用隔离 suite。
...
| 对象 | 固定版本与状态 | 证据能说明什么 |
| --- | --- | --- |
| 上批整理前源与远端（历史） | `codex/inference-foundation`，`b334920907de0324bf3e0146bb78433742356a6c`；本轮读远端确认相同 | 整理阶段只改文档；本轮在其上增加节点说明 UI。不可用该旧版本代表本轮 UI 受测代码 |
...
本阶段到模型说明原型为止。用户正在研究组合方式和新 UI；下一阶段先由用户审阅模型/端口表达，再冻结其具体方案，不先开发组合器。AP1/I2V/CORE仍为独立候选债，首发范围与历史预算保持原记录，不因新增说明页面被标为已修复。
