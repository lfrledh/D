...
当前任务：[D-UI-READINESS-01](tasks/D-UI-READINESS-01.md)，P0—P4有限收尾。用户批准标签N3/N4修复、N1/N6来源与接线回归、N5活动状态去重、P2后端扩展约束与P3现有DRuntime结构演练。只读模型说明页保持，不实现正式节点/图/插件、不批量改后端或项目schema、不恢复旧S6/AP1/CORE/I2V。

本轮源基线`165c54471c4380eb5482312d6c2c70ab97db48a9`，唯一个人scheme差异受保护；隔离组合代码、CPU/hosting、独立普通签名构建及原生GUI已通过。当前进行新上下文接手与源集成门槛核对，尚未推进源或推送。实现、返工、受测/最终版本及恢复点见当前任务末尾和外部回执。上轮模型说明原型已完成并推送；其组合受测`23fc4364c5b04c40db29f25e53a7756611306445`，到165c544仅两份文档变化，本轮复用该历史证据不称重测。

本轮结束后等用户节点UI/UX方案；节点粒度、连接/画布、候选交互和图保存未冻结。开发交接不代表全架构解耦或发布。CURRENT_ACTIONS是当前任务与停点的唯一权威入口，其他活动资料只记录职责/约束与版本范围。
...
- 外盘物理路径/共同Git目录已核对。上一批DATA/VIEW和两个隔离App均已结束；旧full50已回收的记录继续保留。本批TAGS/RUNTIME/CROSS实现及各一次定向修复已停写；本轮两次自有隔离App均正常退出（exit0），Lead继续接手与集成检查；无待用户操作项新增。这是任务级观察，不是系统写锁。
