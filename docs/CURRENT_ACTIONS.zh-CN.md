...
用户要求复杂功能所需的必要接口完成后转模态。音频接口核对没有必须新增项，本批只把两套既有引擎从当前源码和已安装依赖封装到新普通App；不继续谱面/歌声/HUM编辑器。源起点ec7c6eff989dbff501db470838448cc4e793258e，个人scheme保持；独立Lead与P1工作树、规格/模型/写根/保护见[D-AUDIO-CLOSE-VIDEO-01](tasks/D-AUDIO-CLOSE-VIDEO-01.md)。本段是实施中状态，不能当成封装验收或已推送。

[视频设计研究](VIDEO_BACKEND_DESIGN.zh-CN.md)已形成：优先Wan短静音视频路线，先解决整个文本编码器的Mac精度/内存，再扩5B首图；LTX-2.3为大配置音视频候选，许可与资源另验。未下载/安装/运行视频模型，未冻结公共VideoRequest代码。歌声/HUM/TTS为后续独立目标，不再阻塞视频；下面旧“下一阶段紧接歌声”的顺序保留历史。用户已确认普通D/其他AI空闲及解锁；本批Lead串行短音频回归使用隔离新产物，不替换普通D。
