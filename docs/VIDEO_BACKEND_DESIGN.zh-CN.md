...
2026-09-16当前设备决定：本文旧研究不代表当前代码状态。T2V已接纳、I2V仍候选且质量未通过；新用户要求GPU首选/CPU显式备选，覆盖旧CPU VAE作为唯一产品路径的未来指导。VAE是神经网络解码器，非普通MP4解码；GPU/CPU显式选择候选的151组件、真实VAE数值、取消/恢复与CLI已过；小样GPU未更快，效率目标及原I2V画面质量仍未通过。[设备原则与修复清单](EXECUTION_DEVICE_POLICY.zh-CN.md)与[当前I2V任务](tasks/D-VIDEO-I2V-01.md)维护实际状态，不重写下方历史研究。


2026-09-13当前排期：本研究保留为后续技术候选；最新[指导原则](PRODUCT_PRINCIPLES.zh-CN.md)要求先完成有限能力对接，下一阶段提案为[D-ALIGN-01](tasks/D-ALIGN-01.md)。其出口后优先用视频检验新模态扩展，不等全音乐/全工作流编辑器。下文模型资料仍是研究时快照，不是选型冻结或下载/实施授权，本轮没有新增视频运行证据。
