...
2026-09-06 的验证状态：16 项纯框架测试通过，固定模型的真实 CLI 10 类验收通过。MLX XCTest 构建成功，但执行仍等待外盘权限问题解决后复验；连续运行的少量活跃分配增长也正在独立核查，因此 MLX 阶段仍待完整验收。构建命令、真实运行方法和具体限制见 [本地 MLX 参考实现](MLX_REFERENCE_GUIDE.zh-CN.md)。
