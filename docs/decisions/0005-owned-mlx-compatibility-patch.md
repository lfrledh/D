...
日期：2026-09-06。状态：已实施；C++ 回归、真实 CLI 和 50 轮释放验证通过，完整 XCTest 已实际通过 25 项声明／37 个展开场景。详细证据见 [修复记录](../MLX_OWNERSHIP_FIX.zh-CN.md)。
...
完整重编译 Cmlx 与调用方，运行独立数组/纯模型加载实验、真实 CLI 的正常生成与取消、重复推理、旧应用构建。五轮分配容差从 64 MiB 收紧为 1024 bytes，能检测原来的增长。完整 MLX XCTest 单独实际运行通过；后续升级仍须分别复验，C++ 测试和 CLI 不能替代它。
