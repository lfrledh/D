...
> 下方B2表和命令保留该历史阶段范围；当前工作台采用硬件建议与用户参数分离的可扩展profile。能力与实际验收以CURRENT_ACTIONS和对应任务记录为准，不能把历史512尺寸当作整个产品上限。

## 单参考图候选 IR1（2026-09-14，GUI待验收）

当前隔离候选复用同一Klein4B q8权重，在项目的图像参数栏增加“导入PNG”“使用已选图片”“移除参考”。它们明确改变当前草稿输入；浏览、比较或采用一张图片不会暗中改变参考。生成前冻结提示词、参数和参考身份，产生新候选，原图逐字节保留；当前草稿变化不会改写已提交任务。参考是近似语义条件，不保证未描述区域像素不变。

首个adapter支持一张8位RGB/RGBA PNG，参考和输出尺寸各256…2048、32倍数，不隐式缩放裁切。输入经有界解码转换为sRGB、透明部分白底合成；未知色彩/方向、动画、损坏或超限明确拒绝。原PNG保存在项目内，单任务RGB派生输入携带摘要/几何/编码。schema9升级保留旧清单备份，项目移动后按新位置重建任务输入。

公共请求在ImageRequest.referenceImage传入ImageReference，使用referenceKlein4B/1 profile；默认旧profile拒绝参考请求，不能静默丢弃图像条件。MLXImageBackend的scalableKlein4B宿主执行参考VAE编码、条件扩散、输出解码，仍在同一重推理许可中分阶段加载/释放。新原生入口在工作台，本节没有新增CLI参数。每次提交记录实际参考，与当前草稿分别展示。

候选支持同项目复用条件、采用/保存和普通PNG安全导出；现有PNG配方不能打包参考依赖，因此对这些候选明确拒绝完整配方导出。蒙版、编辑强度、多参考、LoRA、RAW/任意JPEG不在IR1范围。组件/真实后端验证与实际GUI闭环分别记录，详情见[任务与证据](tasks/D-IMAGE-REFERENCE-01.md)。
