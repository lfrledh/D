...
日期：2026-09-06。以下保留原版 MLX 0.30.6 / mlx-swift-lm 2.30.6 的问题复现，Apple M4，Debug arm64。随后已实施固定 vendor 修复，最新结果见 [修复报告](../MLX_OWNERSHIP_FIX.zh-CN.md)。
...
这是在最初检查点尚未修复的可重复依赖问题。模型的大块权重能释放、缓存归零，不等于无泄漏。此计数只覆盖 MLX 分配器，不包含全部 CPU 图对象或进程 RSS，不能把 2720 bytes 当作总内存增长的上界，也不能据此保证无限次加载稳定。
...
最初检查点保留了锁定依赖、实测记录和独立复现程序。随后用户授权主动修复依赖，已按 [ADR 0005](../decisions/0005-owned-mlx-compatibility-patch.md) 回移小补丁。没有修改外部 SwiftPM checkout，也没有为全部随机占位权重额外执行昂贵求值，或复制整个模型加载器来掩盖底层问题。后续依赖升级需先复跑此 probe，再复跑真实取消、失败恢复和连续运行验收；问题解决前不把本后端宣称为通过长期内存稳定性验收。
...
```sh
xcodebuild -workspace D.xcworkspace -scheme mlx-allocation-probe -configuration Debug \
  -destination 'platform=macOS,arch=arm64' \
  -derivedDataPath ../BuildCaches/D-MLX \
  -clonedSourcePackagesDirPath ../D-Development/SourcePackages-MLX \
...
本机原始记录：同级 D-Development/Logs/mlx-allocation-probe.log 与 mlx-load-only-probe.log。仅加载模式使用一个位置参数；无参数执行数组对照。当前 checkout 已修复，这些命令预期不再出现原表的增长；旧版原始日志另保存在 Logs/BeforeMLXOwnershipFix。
